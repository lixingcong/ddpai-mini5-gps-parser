...
- PC端Chrome浏览器（Android端可用Firefox或者Yandex）安装允许跨站请求的CORS扩展。在扩展应用商店中搜索CORS，条目有很多，如[Allow CORS](https://mybrowseraddon.com/access-control-allow-origin.html)或者[CORS Everywhere](https://github.com/x1dse/cors-everywhere-firefox-addon)
...
- Firefox移动版，可以在"关于Firefox"中点击5次火狐图标，开启开发者模式，并安装本地xpi插件（CORS）。注意关闭xpi的签名检查（在`about:config`中设置`xpinstall.signatures.required`为false）
